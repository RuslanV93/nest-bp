...
export class PostsSqlQueryRepository {
  ...
  async getPosts(
    ...
    likes_dislikes_count AS ( 
        ...
        FROM "POST_LIKES" pl
        WHERE pl."parentId" = ANY(SELECT fp._id FROM filtered_posts fp)
    ...
    my_status AS (
        ...
        AND pl."parentId" = ANY(SELECT fp._id FROM filtered_posts fp)
    )
    
    SELECT
        (SELECT COUNT(*) FROM filtered_posts) AS "totalCount",
